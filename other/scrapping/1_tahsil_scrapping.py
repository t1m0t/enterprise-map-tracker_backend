...
async def main():
    ...
    schema = {
        ...
        "fields": [
            {"name": "tahsil_name", "selector": "td:nth-child(2) a", "type": "text"},
            {
                "name": "tahsil_census_code",
                "selector": "td:nth-child(3)",
                "type": "text",
            },
            {
                "name": "tahsil_population",
    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        for district in districts:
            result = await crawler.arun(url=district["district_url"], config=run_conf)
